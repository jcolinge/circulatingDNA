...
long int num_coordinate_pairs(FILE*file,int pairs){
    double tmp;
    int tmpch;
    long int mm= 0;
...
int main(int argc,char*argv[])
{
        ...
        strcpy(output_filename,argv[no_arg-argc]);
            ...
            strcpy(input_filename,argv[no_arg-argc]);
                ...
                exit(0);
                        ...
                        }else if(!strcmp(argv[no_arg-argc],"-sx")||
                            !strcmp(argv[no_arg-argc],"--startxat")){
                            --argc;
                        if(!sscanf(argv[no_arg-argc],"%lf",&start_x_at)){
                            log("Error in '-sx' option.");
                            exit(1);
                        }
                        if(start_x_at < 0){
                            log("x starting value cannot be < 0.");
                            exit(1);
                        }
                            }else {
    ...
    mm= num_coordinate_pairs(file,start_x_at<0.0);
    if(mm<nl+nr+1){
    ...
    for(k= 1;k<=mm;k++)fprintf(stdout,"%1.1f\t%1.8f\n",x[k],yf[k]);
    #ifdef UNIX_LIKE_OS
